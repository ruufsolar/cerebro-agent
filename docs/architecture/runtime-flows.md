...
7. Code renders Spanish test prose, inserts `slack_output` with an idempotency key, and
   sends one reply in the original thread using the output UUID as `client_msg_id`.
8. A newer human message cancels an older run before delivery. Successful replies clear
